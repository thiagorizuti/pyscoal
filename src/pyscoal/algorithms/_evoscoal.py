...
class EvoSCOAL(SCOAL):
    ...
    def _check_ind(self,train_data,valid_data,individual,fitness,scores,n_jobs):
        ...
        results = self._check_coclusters(train_data,coclusters,models,n_jobs=1)
    ...
    def _evaluate_fitness_pop(self,train_data,valid_data,population,fitness,scores,n_jobs):
        ...
        fitness = np.array([result[1] for result in results])
    ...
    def _evaluate_fitness_ind(self,train_data,valid_data,individual,fitness,scores,n_jobs):
        ...
        check = self._check_coclusters(train_data,coclusters,models,n_jobs=1)
        models = None
        if  np.all(check):
            row_clusters,col_clusters = coclusters
            n_row_clusters, n_col_clusters = np.unique(row_clusters).size, np.unique(col_clusters).size
            models = self._initialize_models(coclusters)
            models, train_scores = self._update_models(train_data,coclusters,models,n_jobs)
            if self.fitness_function == 'BIC':
                ...
                fitness = self._baesian_information_criterion(train_data,coclusters,models,n_jobs) 
            else:
                scores[0,:n_row_clusters,:n_col_clusters] = self._score_coclusters(valid_data,coclusters,models,n_jobs)
                scores[1,:n_row_clusters,:n_col_clusters] = self._check_coclusters(valid_data,coclusters,models,n_jobs)
    ...
    def _local_search_ind(self,train_data,valid_data,individual,fitness,scores,n_jobs):
        if not np.isnan(fitness):
            coclusters,models = individual
            coclusters = self._update_coclusters(train_data,coclusters,models,n_jobs)
            individual = (coclusters,models)
    ...
    def _print_status(self,gen_count,delta_fitness,population,fitness,rate,infeasible,elapsed_time):
        ...
        sizes = np.array([(np.max(ind[0][0])+1)*(np.max(ind[0][1])+1) for ind in population])
        max_size = sizes.max()
        min_size = sizes.min()
    ...
    def _converge_evoscoal(self,train_data,valid_data,population,max_gen=5,gen_tol=10,tol=0.01,n_jobs=(1,1),verbose=False):
        ...
        population, fitness, scores = self._evaluate_fitness_pop(train_data,valid_data,population,fitness,scores,n_jobs)
        best = np.nanmin(fitness)
        ...
        while not converged:
            ...
            ls_population, ls_fitness,ls_scores = self._evaluate_fitness_pop(train_data,valid_data,ls_population,ls_fitness,ls_scores,n_jobs)
            ...
            mut_population, mut_fitness, mut_scores = self._evaluate_fitness_pop(train_data,valid_data,mut_population,mut_fitness,mut_scores,n_jobs)
        ...
        coclusters, models  = population[np.nanargmin(fitness)]
