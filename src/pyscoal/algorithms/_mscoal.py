...
class MSCOAL(SCOAL):
    ...
    def _converge_mscoal(self,train_data,valid_data,coclusters,models,verbose):
        ...
        converged = iter_count >= self.max_split
        ...
        while not converged:
            ...
            new_coclusters = self._split_row_clusters(valid_data,new_coclusters,models)
            new_models = self._initialize_models(new_coclusters)
            checked = np.all(self._check_coclusters(train_data,new_coclusters,models))
            if checked:
            ...
            new_coclusters = deepcopy(coclusters)
            new_coclusters = self._split_col_clusters(valid_data,new_coclusters,models)
            new_models = self._initialize_models(new_coclusters)
            checked = np.all(self._check_coclusters(train_data,new_coclusters,models))
            if checked:
                new_coclusters,new_models = self._converge_scoal(train_data,new_coclusters,new_models,False)
